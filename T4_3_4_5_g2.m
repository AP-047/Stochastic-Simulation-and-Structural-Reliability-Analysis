...
function [pf, beta] = ExampleClass_quadratic_FORM_g2
...
lambda2 = 1;  % Exponential distribution parameter for X2
...
for i = 1:max_iter
    g = grad_g2(u, s1, lambda2, m1);  % Gradient of g2
    gtg = g' * g;
    gtu = g' * u;
    f = limitState_g2(u, s1, lambda2, m1);  % Limit-state function for g2
...
x2s = exp(u(2)) / lambda2;  % X2 (Exponential backtransformation)
...
% Compute the limit state function g2 over the grid
g_values = zeros(size(u1_grid));
for i = 1:size(u1_grid, 1)
    for j = 1:size(u1_grid, 2)
        g_values(i, j) = limitState_g2([u1_grid(i, j); u2_grid(i, j)], s1, lambda2, m1);
    end
end
...
xlabel('U1');
ylabel('U2');
title('Limit State Function g2 in U-space with P_f');
...
xlabel('X_1');
ylabel('X_2');
title('MPFP in X-space for g_2');
grid on;

end

% Gradient of limit-state function g2
function grad_g2 = grad_g2(u, s1, lambda2, m1)
    x1 = s1 * u(1) + m1;
    x2 = exp(u(2)) / lambda2;
    grad_g2 = [2 * x1; -3 * x2^2];  % Derivatives of g2
end

% Limit-state function g2 = X1^2 - X2^3 + 23
function ls_g2 = limitState_g2(u, s1, lambda2, m1)
    x1 = s1 * u(1) + m1;
    x2 = exp(u(2)) / lambda2;
    ls_g2 = x1^2 - x2^3 + 23;  % Limit-state function g2
end